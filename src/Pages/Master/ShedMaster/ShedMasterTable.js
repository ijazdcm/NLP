  ...
  ShedMaster.getShed().then((response) => {
    ...
    viewData.map((data, index) => {
      rowDataList.push({
        ...
        Shed_Owner_Phone_2: data.shed_owner_phone_2,
        Shed_Owner_Address: data.shed_owner_address,
        Shed_Owner_Photo: (
          <CustomSpanButton handleViewDocuments={handleViewDocuments} shedId={data.shed_id} documentType={'SHED_OWNER_PHOTO'} />
        ),
          ...
          <Link className="text-white" to="/ShedMaster">
            <CButton size="md" color="warning" className="px-3 text-white" type="button">
              <span className="float-start">
                <i className="" aria-hidden="true"></i> &nbsp;New Shed
              </span>
            </CButton>
          </Link>
        </CCol>
